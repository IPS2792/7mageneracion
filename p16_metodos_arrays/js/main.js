...
const priceLess50 = products.filter((product) => {
    return product.price < 50;
})
console.log(priceLess50);

const priceMore50 = products.filter((product) => {
    return product.price >= 50;
})
...
const toSpanish = products.map((product) => {
    if(product.type === 'beer'){
        return `Tipo: Cerveza - Descripción: ${product.description} - Precio: $${product.price}`
    } else if(product.type === 'chip'){
        return `Tipo: Fritura - Descripción: ${product.description} - Precio: $${product.price}`
    } else {
        return `Tipo: ${products.type} - Descripción: ${product.description} - Precio: $${product.price}`
    }
})
